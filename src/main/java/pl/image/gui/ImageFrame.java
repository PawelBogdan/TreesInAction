...
//        fileNameFilter = (File file, String name1) -> true;
    ...
    public ImageFrame(pl.image.Image image) {
        this();
        imageView.setImage(image.getBufferedImage());
    }

    public ImageFrame(String title, pl.image.Image image) {
        this(title);
        imageView.setImage(image.getBufferedImage());
    }
    ...
    public static void display(pl.image.Image img) {
        ImageFrame i = new ImageFrame(img);
        i.display();
    }

    public static void displayCopy(pl.image.Image img) {
        display(img.copy());
    }
